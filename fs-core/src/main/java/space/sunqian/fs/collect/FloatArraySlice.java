...
public class FloatArraySlice implements ArraySlice<float[]> {

    /**
     * Returns a new {@link FloatArraySlice} from the specified array, start index, and end index.
     *
     ...
     * @return the new {@link FloatArraySlice} from the specified array, start index, and end index
     * @throws IndexOutOfBoundsException if the start index or end index is out of range
     */
    public static @Nonnull FloatArraySlice of(
        float @Nonnull [] array,
        int startIndex,
        int endIndex
    ) throws IndexOutOfBoundsException {
        Checker.checkInBounds(startIndex, endIndex, 0, array.length);
        return new FloatArraySlice(array, startIndex, endIndex);
    }

    private final float @Nonnull [] array;
    private final int startIndex;
    private final int endIndex;

    private FloatArraySlice(float @Nonnull [] array, int startIndex, int endIndex) {
        this.array = array;
