...
public class CharArraySlice implements ArraySlice<char[]> {
     ...
     * @return the new {@link CharArraySlice} from the specified array, start index, and end index
     * @throws IndexOutOfBoundsException if the start index or end index is out of range
     */
    public static @Nonnull CharArraySlice of(
        char @Nonnull [] array,
        int startIndex,
        int endIndex
    ) throws IndexOutOfBoundsException {
        Checker.checkInBounds(startIndex, endIndex, 0, array.length);
        return new CharArraySlice(array, startIndex, endIndex);
    }

    private final char @Nonnull [] array;
    private final int startIndex;
    private final int endIndex;

    private CharArraySlice(char @Nonnull [] array, int startIndex, int endIndex) {
        this.array = array;
