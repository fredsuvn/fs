 ...
 * @author sunqian
 */
public interface ByteReaderX {

    /**
     * Returns a new {@link ByteReaderX} with the given input stream. The mark/reset operations are depends on and shared
     * with the input stream.
     *
     * @param source the given input stream
     * @return a new {@link ByteReaderX} with the given input stream
     */
    static @Nonnull ByteReaderX from(@Nonnull InputStream source) {
        return ByteReaderXImpl.of(source);
    }

    /**
     * Returns a new {@link ByteReaderX} with the given byte array.
     * <p>
     * The content of the segment returned from the {@link ByteReaderX#read(int)} is shared with the content of the given
     ...
     * @return a new {@link ByteReaderX} with the given byte array
     */
    static @Nonnull ByteReaderX from(byte @Nonnull [] source) {
        return from(source, 0, source.length);
    }

    /**
     * Returns a new {@link ByteReaderX} with the given byte array, starting at the specified offset and up to the
     * specified length.
     * <p>
     * The content of the segment returned from the {@link ByteReaderX#read(int)} is shared with the content of the given
     * byte array. Any changes to the segment's content will be reflected in the given byte array, and vice versa. The
     ...
     */
    static @Nonnull ByteReaderX from(byte @Nonnull [] source, int offset, int length) throws IndexOutOfBoundsException {
        return ByteReaderXImpl.of(source, offset, length);
    }

    /**
     * Returns a new {@link ByteReaderX} with the given byte buffer.
     * <p>
     * The content of the segment returned from the {@link ByteReaderX#read(int)} is shared with the content of the given
     ...
     * @return a new {@link ByteReaderX} with the given byte buffer
     */
    static @Nonnull ByteReaderX from(@Nonnull ByteBuffer source) {
        return ByteReaderXImpl.of(source);
    }
