...
public interface CharEncoder {
    ...
    /**
     * Returns a new {@link CharEncoder} that wraps the given encoder to encode data in fixed-size blocks. The returned
     ...
     * @return a new {@link CharEncoder} that wraps the given encoder to encode data in fixed-size blocks
     */
    ...
    /**
     * Returns a new {@link CharEncoder} that wraps the given encoder to round down incoming data. The returned encoder
     ...
     * @param encoder the given encoder
     * @return a {@link CharEncoder} to round down incoming data for the given encoder
     */
    static CharEncoder withRounding(int size, CharEncoder encoder) {
        return new CharProcessorImpl.RoundEncoder(encoder, size);
    }

    /**
     * Returns a new {@link CharEncoder} that wraps the given encoder to buffer unconsumed data. It is typically used
     ...
     *
     * @param encoder the given encoder
     * @return a {@link CharEncoder} that buffers unconsumed data of the given encoder
     */
