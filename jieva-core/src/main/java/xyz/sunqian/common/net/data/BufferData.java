...
final class BufferData implements GekData.OfBuffer {
    ...
    public int write(ByteBuffer dest, int length) {
        return 0;//(int) ByteEncoder.from(IOKit.newInputStream(buffer)).readLimit(length).writeTo(dest);
    }
    ...
    public long write(OutputStream dest, long length) {
        return 0;// ByteEncoder.from(IOKit.newInputStream(buffer)).readLimit(length).writeTo(dest);
    }
